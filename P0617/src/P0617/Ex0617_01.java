...
public class Ex0617_01 {
	...
	public static void main(String[] args) {
		// work1 폴더에 txt파일이 몇 개 있는지 출력해 보세요
		// 1. 전체 폴더에 파일을 모두 읽어오기
		// 2. txt 끝나는지 확인 카운트
		// 3. 카운트 출력
		Scanner scan = new Scanner(System.in);
		System.out.println("찾으려고 하는 폴더주소를 입력하세요");
		...
		File[] f_name = fileName.listFiles(); // 폴더안의 모든 파일들 정보를 가져옴.
		// 배열만큼 for돌려서 이름을 비교
		int count = 0; // txt파일 개수
		if (choice == 1) {
			for (int i = 0; i < f_name.length; i++) {
				if (f_name[i].getName().contains("txt")) {
					count++;
				}
			}
		} else {
